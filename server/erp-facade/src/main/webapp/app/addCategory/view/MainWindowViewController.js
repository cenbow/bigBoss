/*
 * File: app/view/MyWindowViewController.js
 *
 * This file was generated by Sencha Architect version 3.5.1.
 * http://www.sencha.com/products/architect/
 *
 * This file requires use of the Ext JS 6.0.x library, under independent license.
 * License of Sencha Architect does not include license for Ext JS 6.0.x. For more
 * details see http://www.sencha.com/license or contact license@sencha.com.
 *
 * This file will be auto-generated each and everytime you save your project.
 *
 * Do NOT hand edit this file.
 */

Ext.define('addCategory.view.MainWindowViewController', {
  extend: 'Ext.app.ViewController',
  alias: 'controller.mainwindow',

  init: function () {
  },

  /**
   * 所属栏目变化
   */
  onColumnChange:function(field, value){
    var viewCtr = this,
        viewModel = viewCtr.getViewModel(),
        view = viewCtr.getView();
    viewModel.set('columnCode',value);

    var store = viewModel.getStore('categorycomboboxstore');
    if (store) {
      //store.removeAll();
      Ext.getCmp('categoryComboxStore').clearValue();
      store.load({
        params: {code: value},
        scope: store
      });
    }
  },

  /**
   * 保存
   */
  onSaveButtonClick: function (button, e, eOpts) {
    var viewCtr = this,
        currWin = viewCtr.getView(),
        viewModel = this.getViewModel(),
        formCmp = currWin.down("form");
    if (!formCmp.isValid()) {
      TipsUtil.showTips("提示", "信息填写有误", TipsUtil.WARING);
      return ;
    }
    var formData = formCmp.getForm().getValues();
    if(!formData.upClassId){
      formData.upClassId = 0;
    }
    var url = FACADE_URL+'/category/add';
      Ext.Ajax.request({
        method: 'POST',
        url: url,
        params: formData,
        success: function (request) {
          if (request.responseText) {
            var json = Ext.decode(request.responseText);
            if (json.success) {
              TipsUtil.showTips("提示", "新增成功", TipsUtil.INFO);
              formCmp.getForm().reset();
              return;
            } else {
              TipsUtil.showTips("错误", json.error.message || "服务器错误！");
            }
          }
        }
      });
  },

  categoryChange:function(field, newValue, oldValue, eOpts){
    var viewModel = this.getViewModel();
    if(!newValue) return;

    var url = FACADE_URL+'/category/view/'+newValue;
    Ext.Ajax.request({
      method: 'GET',
      url: url,
      success: function (request) {
        if (request.responseText) {
          var json = Ext.decode(request.responseText);
          if (json.success) {
            viewModel.set('categoryCode',json.data.code);
            viewModel.set('categoryName',json.data.name);
            return;
          } else {
            TipsUtil.showTips("错误", json.error.message || "服务器错误！");
          }
        }
      }
    });
  },

  roleChange:function(field, newValue, oldValue, eOpts){
    var viewModel = this.getViewModel();

    viewModel.set('roleName',field.lastMutatedValue);
    console.info(field);
  }
});