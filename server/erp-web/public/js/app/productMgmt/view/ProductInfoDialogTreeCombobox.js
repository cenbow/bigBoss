Ext.define("ProductMgmt.view.ProductInfoDialogTreeCombobox", {
  extend: 'Ext.form.field.Picker',
  alias: 'widget.productinfodialogtreecombobox',
  ...
  valueField: 'id',
  treeWidth: 280,
  treeStoreexpand: false,//是否展开所有store
    ...
    me.tree = Ext.create('Ext.tree.Panel', {
      ...
      listeners: {
        load: function (store, records) {
          if (me.treeStoreexpand) {  //如果为真，表示需要一次性加载，如果为假，单步加载
            records.expand(me.treeStoreexpand);
          }
  ...
  setValue: function (valueInit) {
    ...
    var me = this,
      tree = this.tree,
      values = (Ext.isEmpty(valueInit)) ? [] : (function() {
        try {
          return valueInit.split(',');
        } catch (e) {
          return [valueInit + '']
        }
    ...
    if (!me.updateInitFlag) {
      if (me.records.length > 0) {
        me.records[0].parentNode.expand();
        me.tree.getSelectionModel().select( me.records[0]);
        me.tree.getView().focusNode( me.records[0]);
      }
      me.updateInitFlag = true;
    }
